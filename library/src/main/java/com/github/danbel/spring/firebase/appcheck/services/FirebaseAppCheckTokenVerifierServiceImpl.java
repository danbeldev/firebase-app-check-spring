...
import com.github.danbel.spring.firebase.appcheck.FetchFirebaseAppCheckPublicKeyConfig;
...
public class FirebaseAppCheckTokenVerifierServiceImpl implements FirebaseAppCheckTokenVerifierService {
    ...
    /**
     * Конструктор сервиса верификации токенов Firebase App Check.
     *
        ...
        try {
            ...
            JwkProvider jwkProvider = new JwkProviderBuilder(new URL(url))
                    .cached(
                            ...
                            Duration.ofMillis(cacheConfig.expiresIn().toMillis())
                    )
                    .rateLimited(rateLimitedConfig.enabled())
                    .build();
